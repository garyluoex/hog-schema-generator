...
public class StaticProcessor implements Processor {
  ...
  public GeneratedEnvironment generate(GeneratedEnvironment generatedEnvironment) {
    ...
    TypeDefinitionRegistry generatedTypeDefinitionRegistry = generatedEnvironment.getGeneratedTypeDefinitionRegistry();
    ...
    List<InterfaceTypeDefinition> interfaceDefinition =
        abstractTypes
            ...
            .map(
                type ->
                    generatedTypeDefinitionRegistry.getType(type, InterfaceTypeDefinition.class).get())
            .collect(Collectors.toList());
...
//  private ObjectTypeDefinition removeSpecificInterface(ObjectTypeDefinition sourceDefinition) {
//    List<FieldDefinition> newFieldDef = new ArrayList<>(sourceDefinition.getFieldDefinitions());
//
...
//            .getImplements()
//            .stream()
//            .filter(type -> !abstractTypeNames.contains(HelperMethods.serializeType(type)))
//            .collect(Collectors.toList());
//
//    return new ObjectTypeDefinition(
//        sourceDefinition.getName(),
