...
public class YamlInputAdapter implements InputAdapter {
  ...
  private ObjectTypeDefinition generateDatasourcesDefinition(Map<String, InputDatasourceTemplate> inputDatasourceTemplates) {
    String name = "Datasources";
    List<Type> type = Collections.singletonList(new TypeName("HOGSDatasources"));

    List<FieldDefinition> fieldDefinitions = inputDatasourceTemplates.values().stream().map(this::generateDatasourcesFieldDefinition).collect(Collectors.toList());
    return new ObjectTypeDefinition(name, type, Collections.emptyList(), fieldDefinitions);
  }

  private FieldDefinition generateDatasourcesFieldDefinition(InputDatasourceTemplate inputDatasourceTemplate) {

    return new FieldDefinition(inputDatasourceTemplate.getApiName(), new ListType(new TypeName(inputDatasourceTemplate.getApiName()+"Record")));
  }
